"""
Aster Spot API client used by the cash-carry strategy.
"""
...
class AsterSpotClient(BaseSpotExchangeClient):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.error("Aster spot connection error: %s", exc)
            if self._session:
    ...
    async def _request(
        ...
        *,
        ...
        if self.debug:
            logger.info("[ASTER SPOT REQUEST] %s %s %s", method, url, json.dumps({k: v for k, v in params.items() if k != "signature"}))
        ...
        try:
            ...
            raise Exception(f"Aster spot API timeout after {timeout}s") from exc

        if self.debug:
            logger.info("[ASTER SPOT RESPONSE] %s %s", status_code, json.dumps(result)[:2000])

        if isinstance(result, dict) and "code" in result and status_code >= 400:
            raise Exception(f"Aster spot API Error: {result.get('msg', 'Unknown error')} (code: {result.get('code')})")
        return result
    ...
    async def get_order_book(self, symbol: str, limit: int = 20) -> dict[str, Any]:
        spot_symbol = symbol if "/" not in symbol else get_exchange_symbol(symbol, Exchange.ASTERDEX)
    ...
    async def get_symbol_info(self, symbol: str) -> dict[str, Any]:
        ...
        if not symbols:
            raise Exception(f"Aster spot symbol info not found for {spot_symbol}")
        info = symbols[0]
    ...
    async def _round_market_quantity(self, symbol: str, quantity: float) -> float:
        ...
        if min_qty > 0 and rounded < min_qty:
            raise Exception(f"Quantity {rounded} is below Aster spot minQty {min_qty}")
    ...
    async def place_market_order(self, symbol: str, side: str, quantity: float) -> SpotOrder:
        ...
        result = await self._request("POST", "/api/v1/order", params, signed=True)
        return SpotOrder(
            ...
            timestamp=datetime.fromtimestamp(result.get("updateTime", self._get_timestamp()) / 1000, tz=timezone.utc),
            raw_data=result,
        )

    async def transfer_spot_futures(self, asset: str, amount: float, *, to_futures: bool) -> dict[str, Any]:
        params = {
            "asset": asset,
            "amount": amount,
            "clientTranId": f"carry_{int(time.time()*1000)}",
            "kindType": "SPOT_FUTURE" if to_futures else "FUTURE_SPOT",
        }
        return await self._request("POST", "/api/v1/asset/wallet/transfer", params, signed=True)

    async def get_base_asset_balance_for_pair(self, pair: str) -> SpotBalance:
        return await self.get_balance(self._base_asset(get_exchange_symbol(pair, Exchange.ASTERDEX)))
